...
int main(int argc, char const *argv[]) {
  ...
  bool verbose;
  bool doCheck;
  parseCLA(argc, argv, N, NumReps, NumBlocks, NumThredsPBlock, verbose,
           doCheck);
  ...
  fullwork.start();
  auto [h2dv1, kernv1, d2hv1] = benchmarkStream(
    NumReps, N, host, dev, NumBlocks, NumThredsPBlock, verbose);
  fullwork.stop();
